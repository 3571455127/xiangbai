...
require DT_ROOT.'/api/PHPExcel.php';
if(!$MQG) {
...
if($_userid){
	...
	$user2 = $db->get_one("SELECT yqcode FROM {$DT_PRE}member WHERE userid=$_userid");
//print_r($user2);
	extract($user2);
//        echo $yqcode;
	if(!$yqcode) {
                $yqcode = time().rand(0,9);
		$db->query("UPDATE {$DT_PRE}member SET yqcode=$yqcode WHERE userid=$_userid");
		userclean($user2);
	}
}
//会员折扣$salespricea
if($_groupid){
$salesprice2 = $db->get_one("SELECT salesprice as salespricea,level as levelnum FROM {$DT_PRE}member_group WHERE groupid=$_groupid");
extract($salesprice2);
$salespricea = $salespricea/100;
}
//echo $salesprice.'/'.$salespricea.'/'.$vipnum;
