...
//获取wx jsapi
function get_jsapi() {
        $wt = cache_read('weixin-jsapi.php');
        if($wt && $wt['jsapi'] && DT_TIME - $wt['time'] < 7000) return $wt['jsapi'];
        $wttk = cache_read('weixin-token.php');	
        $token = $wttk['token'];
...
//        $arr = $this->http_get2($url);
        ...
        $rec = curl_exec($ch);
         // 关闭cURL资源，并且释放系统资源
            curl_close($ch);
        $result = json_decode($rec, true);
//        print_r($result);eixt;
        $access_jsapi = isset($result['ticket']) ? $result['ticket'] : '';
        cache_write('weixin-jsapi.php', array('time' => DT_TIME, 'jsapi' => $access_jsapi));
        return $access_jsapi;
}
//获取jsapi 签名
function get_signature(){
    $tmpArr = array();
    $nonceStr = $tmpArr['noncestr'] = chr(rand(97,122)).chr(rand(97,122)).chr(rand(97,122)).chr(rand(97,122)).chr(rand(97,122));
    $wtjs = get_jsapi();
    //echo $wtjs.'<br/>';
    $tmpArr['jsapi_ticket'] = $wtjs;
    $timestamp = $tmpArr['timestamp'] =  time();
    //$timestamp = $tmpArr['timestamp'] = '1590402159';
    $tmpArr['url'] = 'https://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
    ksort($tmpArr);
    //echo $nonceStr."<br>";
    //print_r($tmpArr);
    $tmpStr = '';
    ...
    //echo '<br/>'.$tmpSn;
    //exit;
//    $appId = 'wx4f168a1f4d668ef4';
    $WX = cache_read('weixin.php');
    $appId = $WX['appid'];
    $jsarr = array();
    $jsarr['appId'] = $appId;
    $jsarr['nonceStr'] = $nonceStr;
    $jsarr['timestamp'] = $timestamp;
    $jsarr['signature'] = $tmpSn;
    return $jsarr;
}


//查询某张表的某个值
function get_val($wherea, $from = 'title',$tablea) {
	global $db;
//        echo "SELECT `$from` FROM $tablea WHERE $wherea";
	$r = $db->get_one("SELECT `$from` FROM $tablea WHERE $wherea");
	return $r[$from];
}
...
//自定义字段
function get_fields($zhcenter){
    global $db;
    $sql="select * from {$db->pre}fields where front=1 and addition like '%$zhcenter%' order by listorder DESC";
//    echo $sql;
    $data=get_all($sql);
...
//表单字段与值
function get_fielval($zhcenter,$itemid){
        global $db;
        $ta_mode="{$db->pre}fields";
        $sql="select name from $ta_mode where front=1 and addition like '%$zhcenter%' order by listorder DESC";
    //    echo $sql;
        $fzxyuyue=get_all($sql);
    //    print_r($fzxyuyue);
        foreach($fzxyuyue as $key=>$val){
            foreach($val as $k=>$v){
                $str.=$v.',';
            }
        }
        $strle=strlen($str);
        $str=substr($str,0,$strle-1);
        $ta_mode2="{$db->pre}guestbook";
        if(!$str){
            $zdmc='';
            return $zdmc;
        }
        $sql2="select $str from $ta_mode2 where itemid=$itemid";
    //    echo $sql2;
        $bdyuyue=$db->get_one($sql2);
    //    echo $itemid;
    //    echo $str;
       // print_r($bdyuyue);
        //unset($bdyuyue['ftype']);
        //字段名称
        $sql3="select name,title from $ta_mode where front=1 and addition like '%$zhcenter%' order by listorder DESC";
        $zdmc=get_all($sql3);

        //print_r($zdmc);
        foreach($zdmc as $key=>$val){
            foreach($val as $k=>$v){
                foreach($bdyuyue as $kb=>$vb){
                    if($v==$kb){
                        $zdmc[$key][value]=$vb;
                    }
                }
...
//商品字段与值
function get_fielmv($zhcenter,$itemid){
        global $db;
        $ta_mode="{$db->pre}fields";
        $sql="select name from $ta_mode where front=1 and addition like '%$zhcenter%' order by listorder DESC";
    //    echo $sql;
...
function get_fielduo($zhcenter,$itemid){
        global $db;
        $ta_mode="{$db->pre}fields";
        $sql="select name from $ta_mode where front=1 and addition like '%$zhcenter%' order by listorder DESC";
    //    echo $sql;
    ...
    //    print_r($fzxyuyue);
        ...
        $sql2="select $str from $ta_mode2 where itemid in($itemid)";
    //    echo $sql2;
        $bdyuyue=get_all($sql2);
    //    echo $itemid;
    //    echo $str;
//        print_r($bdyuyue);
        //unset($bdyuyue['ftype']);
...
//某张表的字段与值
function get_mfielv($zhcenter,$itemid,$table){
        global $db;
        $ta_mode="{$db->pre}fields";
        $sql="select name from $ta_mode where front=1 and addition like '%$zhcenter%' order by listorder DESC";
    //    echo $sql;
...
//手机短信验证
function dxfasong($shouji,$data){
      $DT_TIME = time(); 
  $word=0;
  $_username='';
    $url = "http://211.147.244.114:9801/CASServer/SmsAPI/SendMessage.jsp?userid=63723&password=123bybx&destnumbers=$shouji&msg=$data";
    //        $url = 'http://121.8.152.210:8611/SF_WebQuery/info.aspx?dc=0007722&inc=&r=0.947072094022163';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_HEADER, 0);
    curl_setopt($ch, CURLOPT_URL,$url);
    ...
    // exit;
     if(!$result[1]){
         $sql="INSERT INTO {$CFG['tb_pre']}sms (mobile,message,word,editor,sendtime,code) VALUES ('$shouji','$data','$word','$_username','$DT_TIME','$code')";
        mysql_query($sql);
        $str='发送成功！';
     }elseif($result[1]==103){
         $str='提交过快！';
...
//搜索20150704
function searcha1($null,$kw,$condition=""){
        //$null为模块id，$kw为搜索关键词,$condition为条件，搜索某个模块某个分类
        global $db;
        global $DT_PRE;
        //我这里只搜索后期添加的模块
        if($null>20 && $null!=23){
            $table=$DT_PRE.'article_'.$null;
        }    
        $fds="a.itemid,a.thumb,a.title,a.linkurl,a.catid,a.addtime,c.moduleid,m.moduledir";
        if($null==26){
           $fds.=",a.xiazai"; 
        }
        if($null==27){
        ...
        $condi = " a.catid=c.catid and c.moduleid=m.moduleid and a.status=3 and a.title like '%$kw%' ";
        if($condition){
            $condi.=$condition;
        }
        $sql="select $fds from {$table} a,{$DT_PRE}category c,{$DT_PRE}module m where $condi order by a.addtime DESC";
                    echo $sql.'<br/>';
        $rel=$db->query($sql);
        if($rel){
            while($a=$db->fetch_array($rel)){
                $list[]=$a;
            }
        }
        return $list;
 ...
 *@pargam $class	样式
*/
function listpages($null='',$count,$current,$limit,$size='3',$class='sabrosus'){
	$str='';
	if($count>$limit){
		...
		//开始
		if($current==1){
                        $str.="<span>共 $count 项  | 共 $pages 页 | 第 $current 页</span>";
...
function jiaoyan($title){
            preg_match_all('/.*select.*/',$title,$aj);
            preg_match_all('/.*delete.*/',$title,$bj);
            preg_match_all('/.*insert.*/',$title,$cj);
            preg_match_all('/.*update.*/',$title,$dj);
            preg_match_all('/.*url.*/',$title,$a1);
            preg_match_all('/.*http:.*/',$title,$a2);    
            if(!empty($aj[0]) || !empty($bj[0]) || !empty($cj[0]) || !empty($dj[0]) || !empty($a1[0]) || !empty($a2[0])){
//                echo "碧池";
                echo "<script type='text/javascript'>
            alert('您输入的格式有误,请仔细核对！');
            history.go(-1);
        </script>";
                exit;
            }
...
function send_sms($mobile, $message, $word = 0, $time = 0) {
	...
	$code = dcurl('http://sms.yige-tech.com/send.php', $data);
...
function userinfo($username, $cache = 1) {
	...
	if($r1) {
		...
		$r4 = DB::get_one("SELECT * FROM ".DT_PRE."member_group WHERE groupid=$r1[groupid]");
		$user = array_merge($r1, $r2, $r3,$r4);
	}
