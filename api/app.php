<?php
/*
	[Destoon B2B System] Copyright (c) 2008-2018 www.yige-tech.com
...
if($DT_MOB['os'] == 'ios') {
	if(preg_match("/^([0-9]{1,})@([a-z0-9]{16,})$/i", $EXT['mobile_ios'])) {
		...
		dheader('https://app.yige-tech.com/get.php?o=ios&u='.$t[0].'&k='.encrypt($url, $t[1]));
	...
	if(preg_match("/^([0-9]{1,})@([a-z0-9]{16,})$/i", $EXT['mobile_adr'])) {
		...
		dheader('https://app.yige-tech.com/get.php?o=adr&u='.$t[0].'&k='.encrypt($url, $t[1]));
