...
//                print_r($TYPE);exit;
		$post['content'] = strip_tags($post['content']);
                if($post['ftype']){
                    ...
                    $post['title'] .= dsubstr($post['xingming'], 30).'-';
                    $post['title'] .= dsubstr($post['dianhua'], 30);
...
//			if(in_array($k, $this->fields)) { $sqlk .= ','.$k; $sqlv .= ",'$v'"; }
        ...
        $sqlk = substr($sqlk, 1);
        $sqlv = substr($sqlv, 1);
//        echo "INSERT INTO {$this->table} ($sqlk) VALUES ($sqlv)";exit;
...
//			if(in_array($k, $this->fields)) { $sqlk .= ','.$k; $sqlv .= ",'$v'"; }
	...
	function edit2($post) {
		...
		$sql = '';
		foreach($post as $k=>$v) {
//			if(in_array($k, $this->fields)) $sqlk .= ",$k='$v'";
                    if($k!='type'){
                         $sqlk .= ",$k='$v'"; 
                    }
		}
        $sqlk = substr($sqlk, 1);
//        echo "UPDATE {$this->table} SET $sqlk WHERE itemid=$post[itemid]";exit;
	    $this->db->query("UPDATE {$this->table} SET $sqlk WHERE itemid=$post[itemid]");
		return true;
	}
	function edit($post) {
		$post = $this->set($post);
