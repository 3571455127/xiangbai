		...
		if($mid > 4 && $itemid) {
			...
			if($t && $t['status'] > 2) {
				...
				if($f) {
					...
					$code = $db->query("UPDATE {$DT_PRE}favorite SET title='$title',url='$url',thumb='$thumb',addtime='$DT_TIME' WHERE itemid=$f[itemid]");
					$ajaxreturn = intval($ajaxreturn);
                                        if($code && $ajaxreturn>0){
                                            $ajaxdatas['status'] = 2;
                                            echo json_encode($ajaxdatas);exit;
                                        }
//                                        echo 111;exit;
