...
//        echo $_username;
        //待付款
        $ordernum[] = $db->get_one("SELECT COUNT(*) AS dfknum FROM {$DT_PRE}order WHERE status=1 and buyer = '$_username'");
        $ordernum[] = $db->get_one("SELECT COUNT(*) AS dfhnum FROM {$DT_PRE}order WHERE status=2 and buyer = '$_username'");
        $ordernum[] = $db->get_one("SELECT COUNT(*) AS dshnum FROM {$DT_PRE}order WHERE status=3 and buyer = '$_username'");
        $ordernum[] = $db->get_one("SELECT COUNT(*) AS dpjnum FROM {$DT_PRE}order WHERE status=4 and buyer = '$_username'");
        $ordernum[] = $db->get_one("SELECT COUNT(*) AS tknum FROM {$DT_PRE}order WHERE status=5 and buyer = '$_username'");
        $ordernums = array();
