			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				if($express && in_array($express, array(1,2,3)) && $MOD['module'] == 'mall') {
					...
					$fee = 0;
                            ...
                            //20201228会员折扣
                                if($salespricea){
                            $amount = $amount*$salespricea;
                                }
                                //查询上级id
                                $sjuserid2 = $db->get_one("SELECT sjuserid FROM {$DT_PRE}member WHERE userid=$_userid");
//                                print_r($sjuserid2);
                                extract($sjuserid2);
//                                echo $sjuserid;exit;
				if(isset($N[$t['username']])) {
					$pid = $N[$t['username']];
					$db->query("INSERT INTO {$DT_PRE}order (mid,mallid,pid,sjuserid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$pid','$sjuserid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_mobile','$status','$fee_name','$fee','$cod')");
					$oid = $db->insert_id();
					...
					$db->query("INSERT INTO {$DT_PRE}order (mid,mallid,sjuserid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$sjuserid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_mobile','$status','$fee_name','$fee','$cod')");
					$pid = $db->insert_id();
					if(!$cod) {
						$N[$t['username']] = $pid;
						$ids .= ','.$pid;
