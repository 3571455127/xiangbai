...
if(isset($salesprice)) $do->salesprice = intval($salesprice);
if(isset($level)) $do->level = intval($level);
	...
	if($submit) {
		...
		if($setting['fee_mode']) {//收费会员
			...
			$do->vip = $vip = $setting['fee'] = 0;
                ...
                $do->salesprice = $salesprice = $setting['salesprice'];
                $do->level = $level = $setting['level'];
		$do->add($setting);
...
class group {
	...
	function add($setting) {
		...
		DB::query("INSERT INTO {$this->table} (groupname,vip,salesprice) VALUES('$this->groupname','$this->vip','$this->salesprice','$this->level')");
...
//            print_r($setting);exit;
		...
		DB::query("UPDATE {$this->table} SET groupname='$this->groupname',vip='$this->vip',listorder='$this->listorder',salesprice='$this->salesprice',level='$this->level' WHERE groupid=$this->groupid");
		cache_group();
		return true;
	}
