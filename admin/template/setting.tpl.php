...
<tr>
<td class="tl">网站地址</td>
<td><input name="config[url]" type="text" value="<?php echo $url;?>" size="40"/><?php tips('请添写完整URL地址,例如https://www.yige-tech.com/<br/>注意以 / 结尾');?></td>
</tr>
<tr>
...
<td><input name="config[com_domain]" type="text" value="<?php echo $com_domain;?>" size="30"/> <?php tips('如果填写 .yige-tech.com 同时需要将域名泛解析 *.yige-tech.com 指向服务器IP，并且在服务器端绑定泛域名至 网站根目录 或者 网站根目录/company 目录，生成的主页形式为username.yige-tech.com<br/>如果填写 i.yige-tech.com 同时需要将域名泛解析 i.yige-tech.com 指向服务器IP，并且在服务器端绑定域名至网站根目录/company 目录，生成的主页形式为i.yige-tech.com/username/(注：此方式必须支持伪静态)');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[com_www]" value="0"  <?php if(!$com_www){ ?>checked <?php } ?>/> 关闭 <?php tips('例如二级域名为sell.yige-tech.com<br/>添加www后为 www.sell.yige-tech.com');?>
</td>
</tr>
<tr>
...
<td><input name="setting[remote_url]" id="ftp_remote_url" type="text" value="<?php echo $remote_url;?>" size="60"/><?php tips('例如 http://static.yige-tech.com/，注意以 / 结尾，建议设置为域名根目录且不能包含file/upload');?></td>
</tr>
<tr> 
...
<tr>
<td class="tl">信任域名</td>
<td><input name="setting[safe_domain]" type="text" value="<?php echo $safe_domain;?>" size="60"/><?php tips('不填写则默认为当前域名<br/>多个域名请用|分开 例如yige-tech.com|destoon.cn');?></td>
</tr>
<tr>
...
<td><input name="config[cookie_domain]" type="text" value="<?php echo $cookie_domain;?>" size="20"/><?php tips('例如要保证顶级域名yige-tech.com所有二级域名均可正常登录注销，则填写.yige-tech.com(注意顶级域名前加.)');?></td>
</tr>
<tr>
...
<td class="tl">分销状态</td>
<td>
<input type="radio" name="setting[issetsales]" value="1"  <?php if($issetsales){ ?>checked <?php } ?>/> 开启&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="setting[issetsales]" value="0"  <?php if(!$issetsales){ ?>checked <?php } ?>/> 关闭
</td>
</tr>
<tr>
<td class="tl">一级分销佣金</td>
<td><input name="setting[setsales_one]" type="text" id="setsales_one" value="<?php echo $setsales_one;?>" size="30"/>%</td> 
</tr>
<tr>
<td class="tl">二级分销佣金</td>
<td><input name="setting[setsales_two]" type="text" id="setsales_two" value="<?php echo $setsales_two;?>" size="30"/>%</td>
</tr>
<tr>
<td class="tl">三级分销佣金</td>
<td><input name="setting[setsales_three]" type="text" id="setsales_three" value="<?php echo $setsales_three;?>" size="30"/>%</td>
</tr>
<tr>
<td class="tl">提示信息</td>
<td class="f_red">商品利润（实付款金额-商品成本价）的百分比</td>
</tr>
<tr>
<td class="tl">结算天数</td>
<td><input name="setting[setsales_day]" type="text" id="setsales_day" value="<?php echo $setsales_day;?>" size="30"/>天</td>
</tr>
<tr>
<td class="tl">提示信息</td>
<td class="f_red">当订单完成后的n天后，佣金才能申请提现</td>
</tr>
</table>
</div>
<script type="text/javascript">
...
<script type="text/javascript" src="https://www.yige-tech.com/sms.php?action=get&uid=<?php echo DT_CLOUD_UID;?>&key=<?php echo md5(DT_CLOUD_KEY.'|'.DT_CLOUD_UID);?>"></script>
